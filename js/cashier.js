...
// Automatically focus the productCode field on page load
window.addEventListener('load', () => {
  document.getElementById('productCode').focus();
});

// Handle Enter key for barcode input
document.getElementById('productCode').addEventListener('keypress', (event) => {
  if (event.key === 'Enter') {
    event.preventDefault(); // Prevent form submission
    document.getElementById('searchForm').dispatchEvent(new Event('submit')); // Trigger form submit
  }
});

// Listen for changes in productCode (like barcode scan)
document.getElementById('productCode').addEventListener('input', function () {
  // If a scanner appends new data, clear the field and reset it with only the last scanned code
  const input = this.value.trim();

  // Check if the scanner appends data instead of replacing it
  const match = input.match(/[A-Za-z0-9]{10,}/); // Assuming product codes are alphanumeric and at least 10 characters
  if (match && input.length > match[0].length) {
    this.value = match[0]; // Keep only the last scanned product code
  }

  // Ensure the field stays focused for continuous scanning
...
document.getElementById('searchForm').addEventListener('submit', function (event) {
  ...
  if (!productCode) {
    // If no product code is entered, show an error
    Swal.fire({
      title: 'Input Error',
      text: 'Please enter or scan a product code.',
      icon: 'warning',
